...
> ⚠️ **Important: Total Commander must run as Administrator**, otherwise
> the panel stays empty (Windows silently returns an empty but
> error-free result without elevation - see below for details). Since
> v1.4 the plugin shows an explanatory hint file in that case instead.
...
### Why the status column lives inside the WFX itself

This started out as a separate content plugin (WDX). Testing showed that
Total Commander does **not** automatically call an independent WDX's
value function for files shown through a WFX's own virtual file system
in a live custom-columns view - confirmed both by the official TC forum
thread *"Custom columns in wfx plugins"* and by the reference plugin
*EverySearch*, which exports `FsContentGetSupportedField`/
`FsContentGetValue` in addition to the normal `Content*` functions for
exactly this reason. Since then the status column is implemented
directly in the WFX; a separate WDX is no longer needed.
